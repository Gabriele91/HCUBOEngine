...
class base_texture_specular_material : public material, public smart_pointers<base_texture_specular_material>
{
public:

	base_texture_specular_material()
	{
	}

	base_texture_specular_material(resources_manager& resources)
	{
		init(resources);
	}

	base_texture_specular_material(shader::ptr base_shader)
	{
		init(base_shader);
	}
	...
	void init(resources_manager& resources)
	{
		init(resources.get_shader("base_texture_specular"));
	}
	...
	void init(shader::ptr base_shader)
	{
		...
		//get texture uniform
		m_uniform_texture  = m_shader->get_shader_uniform_texture("texture_id");
		m_uniform_specular = m_shader->get_shader_uniform_texture("specular_id");
		//matrix
		m_uniform_projection = m_shader->get_shader_uniform_mat4("projection");
		m_uniform_view = m_shader->get_shader_uniform_mat4("view");
		m_uniform_model = m_shader->get_shader_uniform_mat4("model");
	...
	void set_specular(texture::ptr tex)
	{
		m_specular = tex;
	}
	...
	virtual void bind(camera* cam, const glm::mat4& model)
	{
		...
		m_uniform_color->set_value(m_color);
		m_uniform_texture->set_value(m_texture);
		m_uniform_specular->set_value(m_specular);
		m_uniform_projection->set_value(cam->get_projection());
		m_uniform_view->set_value(cam->get_view());
		m_uniform_model->set_value(model);
	}
...
private:
	...
	texture::ptr         m_texture;
	uniform_texture::ptr m_uniform_texture{ nullptr };
	texture::ptr         m_specular;
	uniform_texture::ptr m_uniform_specular{ nullptr };
	//model info
	uniform_mat4::ptr m_uniform_projection{ nullptr };
	uniform_mat4::ptr m_uniform_view{ nullptr };
	uniform_mat4::ptr m_uniform_model{ nullptr };
...
class base_texture_normal_material : public material, public smart_pointers<base_texture_normal_material>
{
public:
    
    base_texture_normal_material()
    {
    }
    
    base_texture_normal_material(resources_manager& resources)
    {
        init(resources);
    }
    
    base_texture_normal_material(shader::ptr base_shader)
    {
        init(base_shader);
    }
    ...
    void init(resources_manager& resources)
    {
        init(resources.get_shader("base_texture_normal"));
    }
    ...
    void init(shader::ptr base_shader)
    {
        ...
        //get texture uniform
        m_uniform_texture = m_shader->get_shader_uniform_texture("texture_id");
        m_uniform_normal  = m_shader->get_shader_uniform_texture("normal_id");
        //matrix
        m_uniform_projection = m_shader->get_shader_uniform_mat4("projection");
        m_uniform_view = m_shader->get_shader_uniform_mat4("view");
        m_uniform_model = m_shader->get_shader_uniform_mat4("model");
    ...
    void set_normal(texture::ptr tex)
    {
        m_normal = tex;
    }
    ...
    virtual void bind(camera* cam, const glm::mat4& model)
    {
        ...
        m_uniform_color->set_value(m_color);
        m_uniform_texture->set_value(m_texture);
        m_uniform_normal->set_value(m_normal);
        m_uniform_projection->set_value(cam->get_projection());
        m_uniform_view->set_value(cam->get_view());
        m_uniform_model->set_value(model);
    }
...
private:
    ...
    uniform_texture::ptr m_uniform_normal{ nullptr };
    //model info
    uniform_mat4::ptr m_uniform_projection{ nullptr };
    uniform_mat4::ptr m_uniform_view{ nullptr };
    uniform_mat4::ptr m_uniform_model{ nullptr };
...
class base_texture_normal_specular_material : public material, public smart_pointers<base_texture_normal_specular_material>
{
public:

	base_texture_normal_specular_material()
	{
	}

	base_texture_normal_specular_material(resources_manager& resources)
	{
		init(resources);
	}

	base_texture_normal_specular_material(shader::ptr base_shader)
	{
		init(base_shader);
	}
	...
	void init(resources_manager& resources)
	{
		init(resources.get_shader("base_texture_normal_specular"));
	}
