...
public:
	...
	void build_from_triangles(std::vector< glm::vec3 > &points, std::vector<int> &triangles);

	// builds an OBB from triangles specified as an array of
	// points 3 by 3
	void build_from_sequenzial_triangles(std::vector< glm::vec3 > &points);

	// builds an OBB from points specified as an array.
	void build_from_points(std::vector< glm::vec3 >& points);
...
private:
	...
	void build_from_covariance_matrix(glm::mat3 C, const std::vector< glm::vec3 >& points);
};